...
date: 2018-01-18
redirect_from: ["/procedural-macros-in-rust/"]
...
Fairly recently i started using `macro_rules!()`[^macro_rules]. This is a macro in itself and makes it both simple to 
...
[^macro_rules]:
    By the way: macro_rules!() is not the only useful macro exposed to developers within the standard library. Probably the most written Rust statement, `println!()`, is also a macro!  
...
Although this promises great power, there are still issues preventing a stable implementation of procedural macros[^procedural_macros]. 
